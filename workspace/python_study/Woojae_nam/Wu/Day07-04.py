...
def excelData03_1():
    ...
    for worksheet in workbook.sheets():
        ...
        # Worksheet --> csvList
        if worksheet == workbook.sheets()[0]:
            for i in range(sRow):
                tmpList = []
                for k in range(sCol):
                    value = worksheet.cell_value(i, k)
...
def excelData01_1() :
    ...
    file_list = glob.glob(os.path.join(dirName, "*.xlsx"))  # 선택 폴더의 엑셀파일들(워크북들) 읽기.
    for input_file in file_list:    # 파일 목록 읽기
        filereader = open(input_file, 'r', newline='')
        workbook = xlrd.open_workbook(input_file) # 각 파일 열어 읽기
        sheetNameList = []
        for worksheet in workbook.sheets(): # 각 워크북(파일)의 시트 목록 읽기
            sheetNameList.append(worksheet.name)
        # print(sheetNameList)
        csvList.append([os.path.basename(input_file), '-->', sheetNameList]) #os.path.basename:각 파일의 저장위치를 파일명으로 변환-->각 파일의 시트 목록

        filereader.close()

        ##################################
    # 내장 함수:시트 선택하기(but, 마지막 파일의 시트 목록만 출력됨. 보완 필요!)
    def selectSheet():
        selectedIndex = listbox.curselection()[0]
        subWindow.destroy()
        sheet1 = workbook.sheets()[selectedIndex]
...
excelMenu.add_command(label='Excel정보 보기 - Sheets of a Workbook', command=excelData01)
excelMenu.add_command(label='Excel정보 보기 - Sheets & Workbooks in a Folder', command=excelData01_1)
...
excelMenu.add_command(label='Excel내용 보기 - All with 1 title', command=excelData03_1)
excelMenu.add_command(label='Excel내용 보기 - Last', command=excelData04)
