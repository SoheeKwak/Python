...
def mysqliteData01() :
    ...
    def selectTable() :
        ...
        # 테이블의 열 목록 뽑기
        colNameList = []
        sql = "desc" + tableNameList[selectedIndex] # desc: description
        cur.execute(sql)
        while True:
            row = cur.fetchone()
            if row == None:
...
mysqliteMenu = Menu(mainMenu)
mainMenu.add_cascade(label='MySQLite 데이터 분석', menu=mysqliteMenu)
mysqliteMenu.add_command(label='MySQLite 정보 읽기', command=mysqliteData01)
mysqliteMenu.add_command(label='MySQLite 정보 쓰기', command=mysqliteData02)
