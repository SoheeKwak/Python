...
def  saveJSON() :
    ...
    saveFp = asksaveasfile(parent=window, mode='w', defaultextension='.json',
               filetypes=(("JSON파일", "*.json"), ("모든파일", "*.*")))  #모든파일(csv포함)을 열어 json으로 저장
    filewriter = open(saveFp.name, 'w', newline='')
    # csvList --> jsonDic
    fname = os.path.basename(input_file).split(".")[0] # supplier_data.csv를 split해서
    # print(fname)                                       # supplier_data로 목록 추출
    jsonDic = {}
    jsonList = []
    tmpDic = {}
    header_list = csvList[0] #['Supplier Name', 'Invoice Number', 'Part Number', 'Cost', 'Purchase Date']
    print(csvList)
