    ...
    for i in range(xSize):
        for k in range(ySize):
            ...
            if g in gDic:
                ...
                gDic[g] = 1
    # print(rDic)
    countList = sorted(gDic.items(), key=operator.itemgetter(1))
    print('최소출현 g픽셀값:',countList[0])
    print('최다출현 g픽셀값:', countList[-1])
    for i in range(xSize):
        for k in range(ySize):
            r,g,b=photo.get(i,k)
            if b in bDic:
                ...
                bDic[b] = 1
    countList = sorted(bDic.items(), key=operator.itemgetter(1))
    print('최소출현 b픽셀값:',countList[0])
    print('최다출현 b픽셀값:', countList[-1])
