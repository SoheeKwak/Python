...
def a_histogram() :  # 히스토그램
    ...
    for i in range(0, 256):
        for k in range(0, int(normalList[i])):
            data = 0
            subPaper.put('#%02x%02x%02x' % (data, data, data), (i,256-k)) #거꾸로 나오는 화면을 돌리기 위해 k대신 256-k
...
window.title('영상 처리&데이터 분석 Ver 0.8')
