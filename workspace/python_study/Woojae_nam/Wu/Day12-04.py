...
def a_histoEqual() :  # 히스토그램 평활화 알고리즘
    ...
    for  i  in  range(len(histo)) :
        sVal += histo[i]
        sumHisto[i] = sVal
    # 정규화된 누적 히스토그램 작성 : (누적합 / (행개수*열개수)) * HIGH
...
pixelMenu.add_command(label='영상합성(모핑)', command=morphing)
...
analyzeMenu.add_command(label='히스토그램 스트레칭', command=a_histoStretch)
