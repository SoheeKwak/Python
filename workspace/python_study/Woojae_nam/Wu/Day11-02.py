...
def openSQLite() :
    ...
    cur = con.cursor()  # 연결 통로 생성 (쿼리문을 날릴 통로)
    try :
        sql = "SELECT DISTINCT filename, resolution FROM imageTable" # DISTINCT: 중복된 목록은 하나만 추출
        cur.execute(sql)
        tableNameList = [] #['강아지:128', '강아지:512...]이런식으로 넣음
        while True :
            row = cur.fetchone()
            if row == None:
                break
        ...
        def selectTable():
            ...
            cur.close()  # DB를 먼저 끊고 다음 작업 진행하는 게 효율적
            con.close()  # DB를 먼저 끊고 다음 작업 진행하는 게 효율적
            equal()
            print("OK.")
...
def saveMySQL() :
    ...
    try:
        sql = "DELETE FROM imageTable WHERE filename='" + \
               fname + "' AND resolution=" + str(outW) # 새로운 이미지가 형성되었음에도 파일명이 같게, 이전의 것 뒤로 저장되므로 이전의 파일(맨앞)로만 출력되는 버그 발생. 때문에 이전의 것은 delete하는 명령을 내림
        cur.execute(sql)
        con.commit()
    except:
        pass
...
otherMenu.add_command(label='SQLite로 가져오기', command=openSQLite)
otherMenu.add_separator()
otherMenu.add_command(label='MySQL로 내보내기', command=saveMySQL)
otherMenu.add_command(label='MySQL로 가져오기', command=openMySQL)
