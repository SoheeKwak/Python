...
def max_min():
    ...
    subWindow = Toplevel(window)  # 부모(window)에 종속된 서브윈도
    subWindow.geometry('200x100')
    label1 = Label(subWindow, text='빈도수 최소값 -->' + str(countList[0]));
    label1.pack()
    label2 = Label(subWindow, text='빈도수 최대값 -->' + str(countList[-1]));
    label2.pack()
    subWindow.mainloop()
    print('최소/최다 빈도수:', countList[0], countList[-1])
...
analyzeMenu.add_command(label='빈도수 최대값', command=max_min)
