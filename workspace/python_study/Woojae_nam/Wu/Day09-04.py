...
def sqliteData01(): # 내 pc안의 csv파일 등을 열어서(openCSV), SQL정보쓰기를 하면 userDB에 저장됨. 그 저장된 파일을 불러 옴
    ...
    def selectTable():
        ...
        sql = "SELECT * FROM " + tableNameList[selectedIndex] # 테이블 내용 보기
        cur.execute(sql)
        while True:
            row = cur.fetchone()
            if row == None:
...
def sqliteData02():
    ...
    try:
        ...
        for colName in colList:
            sql += colName + " CHAR(20),"
        sql = sql[:-1] # 맨마지막 CHAR(20), 쉼표는 제거
        sql += ")"
        cur.execute(sql)
    except:
        pass

    for i in range(1, len(csvList)): # head행(즉, 열이름)은 위에서 추출했으므로, 이것을 제외한 다음 행부터 추출
        rowList = csvList[i]
